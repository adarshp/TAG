...
import { getCssRules, sortForSlotting } from "./util.mjs";

import { Word } from "./components/word.mjs";
import { WordCluster } from "./components/word-cluster.mjs";
...
class Main {
  ...
  init() {
    ...
    this.words.forEach((word) => {
      ...
      this.config.topTagCategories.forEach((category) => {
        word.setTopTagCategory(category);
      });

      this.config.bottomTagCategories.forEach((category) => {
