...
$(document).ready(function () {
  var currentSectionNav, target; // If an anchor hash is in the URL highlight the menu item

  highlightActiveHash(); // If a specific page section is in the URL highlight the menu item

  highlightActiveSection(); // If a specific page section is in the URL scroll that section up to the top
...
function highlightActiveHash(event) {
  var oldUrl, oldSubSectionElement; // check for and remove old hash active state
...
function getCurrentSectionName() {
  ...
  var sectionName = pageUrl.substring(0, pageUrl.indexOf(".")); // remove the wodr module- if its in the url
